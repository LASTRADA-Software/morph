...
struct EBWideModel {
    bool execute(const EBWideAction&) { return true; }
};
...
struct EBNarrowModel {
    bool execute(const EBNarrowAction&) { return true; }
};
...
TEST_CASE("schemaJson emits exact text companions for int64 bounds", "[forms][bounds]") {
    ...
    CHECK(schema.find(R"("x-exactMaximum":"9223372036854775807")") != std::string::npos);
    CHECK(schema.find(R"("x-exactMinimum":"-9223372036854775808")") != std::string::npos);
}

TEST_CASE("schemaJson emits an exact text companion for a uint64 maximum", "[forms][bounds]") {
    auto const schema = morph::forms::schemaJson<EBWideAction>();
    CHECK(schema.find(R"("x-exactMaximum":"18446744073709551615")") != std::string::npos);
}
...
TEST_CASE("schemaJson leaves bounds a double holds exactly untouched", "[forms][bounds]") {
    ...
    CHECK(schema.find("x-exactMinimum") == std::string::npos);
    CHECK(schema.find("x-exactMaximum") == std::string::npos);
}
