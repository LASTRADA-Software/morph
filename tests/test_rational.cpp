...
// Arithmetic.
static_assert(Rational{Numerator{1}, Denominator{3}, dp9} + Rational{Numerator{1}, Denominator{3}, dp9} +
                  Rational{Numerator{1}, Denominator{3}, dp9} ==
              Rational::one(dp9));
static_assert(Rational{Numerator{1}, Denominator{10}, dp9} + Rational{Numerator{2}, Denominator{10}, dp9} ==
              Rational{Numerator{3}, Denominator{10}, dp9});
static_assert(Rational{Numerator{5}, Denominator{6}, dp9} - Rational{Numerator{1}, Denominator{6}, dp9} ==
              Rational{Numerator{2}, Denominator{3}, dp9});
static_assert(Rational{Numerator{2}, Denominator{3}, dp9} * Rational{Numerator{3}, Denominator{5}, dp9} ==
              Rational{Numerator{2}, Denominator{5}, dp9});
static_assert((Rational{Numerator{1}, Denominator{7}, dp9} * Rational{Numerator{7}, Denominator{1}, dp9}) ==
              Rational::one(dp9));
...
static_assert(*(Rational{Numerator{1}, Denominator{2}, dp9} / Rational{Numerator{1}, Denominator{4}, dp9}) ==
              Rational{2, dp9});
...
static_assert(*Rational{Numerator{3}, Denominator{4}, dp9}.reciprocal() ==
              Rational{Numerator{4}, Denominator{3}, dp9});
static_assert(*Rational{Numerator{-3}, Denominator{4}, dp9}.reciprocal() ==
              Rational{Numerator{-4}, Denominator{3}, dp9});
...
static_assert(std::same_as<decltype(Rational{} - Rational{}), Rational>);
static_assert(std::same_as<decltype(Rational{} * Rational{}), Rational>);
static_assert(std::same_as<decltype(Rational{} / Rational{}), std::expected<Rational, RationalError>>);

// Mixed-type result types.
static_assert(std::same_as<decltype(std::declval<Rational>() + 3.5), std::expected<Rational, RationalError>>);
static_assert(std::same_as<decltype(std::declval<std::expected<Rational, RationalError>>() + std::declval<Rational>()),
...
static_assert(
    std::same_as<decltype(std::declval<Rational>() / std::declval<Rational>() + std::declval<Rational>() * 3.5),
                 std::expected<Rational, RationalError>>);
static_assert(std::same_as<decltype(3.5 + std::declval<Rational>()), std::expected<Rational, RationalError>>);
...
TEST_CASE("Rational::DenominatorAlwaysPositive", "[rational]") {
    auto const samples = std::array{
        ...
        Rational{Numerator{2}, Denominator{-4}, dp9},
        Rational{Numerator{-2}, Denominator{-4}, dp9},
        Rational{Numerator{5}, Denominator{0}, dp9},
        Rational::zero(dp9),
        Rational::one(dp9),
    };
...
TEST_CASE("Rational::fromFloat::ErrorPaths", "[rational]") {
    ...
    auto const negativeInfinity = Rational::fromFloat(-std::numeric_limits<double>::infinity(), dp6);
...
TEST_CASE("Rational::ExactArithmetic", "[rational]") {
    CHECK(Rational{Numerator{1}, Denominator{3}, dp9} + Rational{Numerator{1}, Denominator{3}, dp9} +
              Rational{Numerator{1}, Denominator{3}, dp9} ==
          Rational::one(dp9));
    CHECK(Rational{Numerator{1}, Denominator{10}, dp9} + Rational{Numerator{2}, Denominator{10}, dp9} ==
          Rational{Numerator{3}, Denominator{10}, dp9});
    CHECK(Rational{Numerator{1}, Denominator{7}, dp9} * Rational{7, dp9} == Rational::one(dp9));
    CHECK(Rational{Numerator{5}, Denominator{6}, dp9} - Rational{Numerator{1}, Denominator{6}, dp9} ==
          Rational{Numerator{2}, Denominator{3}, dp9});
    // Negative left operand exercises the sign handling in the cross-cancel.
    CHECK(Rational{Numerator{-2}, Denominator{3}, dp9} * Rational{Numerator{3}, Denominator{2}, dp9} ==
          -Rational::one(dp9));
...
TEST_CASE("Rational::DivisionByZero", "[rational]") {
    ...
    auto const chained = Rational::from(Numerator{1}, Denominator{2}, dp9).and_then([](Rational value) {
        return value.dividedBy(Rational::zero(dp9));
    });
...
TEST_CASE("Rational::OverflowReduction", "[rational]") {
    ...
    auto const cancelling =
        Rational{Numerator{large}, Denominator{7}, dp9} * Rational{Numerator{7}, Denominator{large}, dp9};
...
TEST_CASE("Rational::Mixed::ExactValuesHappy", "[rational]") {
    ...
    auto const composed = Rational{Numerator{1}, Denominator{2}, dp9} / Rational{2, dp9} +
                          Rational{Numerator{1}, Denominator{4}, dp9} * 2.0;
...
TEST_CASE("Rational::CrossPrecision::MaxPropagates", "[rational]") {
    ...
    CHECK((Rational{Numerator{1}, Denominator{2}, dp3} + Rational{Numerator{1}, Denominator{4}, dp9})
              .getDecimalPlaces() == dp9);
    CHECK((Rational{Numerator{1}, Denominator{2}, dp9} + Rational{Numerator{1}, Denominator{4}, dp3})
              .getDecimalPlaces() == dp9);
    CHECK((Rational{Numerator{1}, Denominator{2}, dp3} + Rational{Numerator{1}, Denominator{4}, dp3})
              .getDecimalPlaces() == dp3);
    CHECK((Rational{Numerator{1}, Denominator{2}, dp3} - Rational{Numerator{1}, Denominator{4}, dp9})
              .getDecimalPlaces() == dp9);
    CHECK((Rational{Numerator{1}, Denominator{2}, dp3} * Rational{Numerator{1}, Denominator{4}, dp9})
              .getDecimalPlaces() == dp9);
...
TEST_CASE("Rational::Mixed::FloatInheritsRationalPrecision", "[rational]") {
    static_assert(std::same_as<decltype(Rational{} + 3.5), std::expected<Rational, RationalError>>);
    static_assert(std::same_as<decltype(3.5 + Rational{}), std::expected<Rational, RationalError>>);
...
TEST_CASE("Rational::RoundToDecimalPlaces::NonTerminatingAndIntegers", "[rational]") {
    ...
    CHECK(roundToDecimalPlaces(third, DecimalPlaces{99}).getDecimalPlaces() == DecimalPlaces{kMaxDecimalPlaces});
}
