...
> `Choice` type of [choice.md](../spec/choice.md) and the `x-option*` schema
> vocabulary of [forms.md](../spec/forms.md). It describes the intended behavior;
...
empty body** ([choice.md](../spec/choice.md)): the renderer calls
...
[choice.md](../spec/choice.md).

## Design

### Extending `Choice` with a parent dependency

`Choice`'s options-metadata NTTPs ([choice.md](../spec/choice.md)) gain an
...
parent selection** — no new dispatch mechanism, exactly as [choice.md](../spec/choice.md)
...
- When a parent value **changes**, the renderer re-fetches with the new body and
  **clears** any existing child selection that is not present in the new result —
  closing (client-side) the staleness that [choice.md](../spec/choice.md) Failure
...
`mergeSchemaExtras` ([forms.md](../spec/forms.md)) already reads a `Choice`
property's compile-time metadata to emit `x-optionsAction` / `x-optionValue` /
`x-optionLabel` on the property node ([choice.md](../spec/choice.md)). It gains
...
options action's *result* field names ([choice.md](../spec/choice.md)).

## Additivity and renderer fallback

`x-optionsDependsOn` is an additive, optional `x-*` key, consistent with the
unversioned-schema stance of [forms.md](../spec/forms.md) and
...
([choice.md](../spec/choice.md)). The child list is then unfiltered (or empty, if
...
guarantee [forms.md](../spec/forms.md) makes.

## Non-goals

- **No server-side option-membership enforcement.** As in
  [choice.md](../spec/choice.md), the wire value is a bare nullable `T` and the
...
- **Parents are flat siblings only.** A `DependsOn` name must resolve to a
  ...
  scope of [forms.md](../spec/forms.md); it cannot reach into a nested aggregate
  or another action.
...
A dependent `Choice` adds to the three obligations [choice.md](../spec/choice.md)
already lists:
...
- [gui_overview.md](gui_overview.md) — the umbrella program; this is its Tier-1
  ...
  overview names.
- [choice.md](../spec/choice.md) — the base `Choice`/`FixedString` design this
...
- [forms.md](../spec/forms.md) — `mergeSchemaExtras` and the property-node `x-*`
...
- [bridge.md](../spec/bridge.md) — the action-execute path the renderer uses to
