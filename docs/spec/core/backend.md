...
- **A protocol violation drops the connection without sending a Close frame.**
  ...
  than 125 bytes, is a Close frame whose status code is not one RFC 6455
  §7.4.1 allows on the wire, uses a non-minimal extended-length encoding, or
  is a text message whose payload is not valid UTF-8. Each of those throws out
