...
- **Toolchain note carried from `scripts/coverage.sh` and `scripts/mutation.sh`:** all baseline numbers in this plan were measured on clang 22.1.8; CI pins clang 20 <!-- ci-clang-pin: historical --> at the time this plan was written (CI has since moved on). Numbers may drift a fraction of a point on re-measurement — that's expected, not a sign the audit was wrong.
