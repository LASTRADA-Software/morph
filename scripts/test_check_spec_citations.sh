...
#
# Self-test for scripts/check_spec_citations.sh's section-citation check
# (check 5, morph#316), the gate that keeps a citation naming a *section* of a
# markdown file pointing at a section that is really there.
#
...
# mutation goes into the *second* occurrence of that file list, which is the
# section-citation scan's; the first is check 3's, with a floor of its own.
break_section_scan() {
    awk '
