...
class ActionExtractionTest(unittest.TestCase):
    ...
    def test_real_tree_pins_the_known_action_names(self) -> None:
        ...
        # `ListTransactions` joined it in morph#428, for the same reason one
        # level down: `UndoTransaction` takes a `journalId` that no result type
        # in the rung ever returned, so the reversal it performs -- and the
        ...
        # bank is not a ladder rung (absent from examples/rungs.txt, never calls
        # morph_add_rung) but ships a src/server/main.cpp, which is what
        # SERVER_RUNGS has always actually selected on. Its 41 registrations are
