...
/// @file
/// The scaffolding every crm test needs: a scoped authenticated principal.
///
/// Models in this rung refuse to mutate anything without one
/// (`crm::requirePrincipal`), so almost every test declares one — same
/// pattern as `lims::test::ScopedPrincipal`.
