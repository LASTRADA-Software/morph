...
TEST_CASE("NotificationModel posts, lists, and marks read", "[notification]") {
    ...
    auto first = await(notes.execute(bank::dto::Notify{.message = "Low balance", .severity = 1}), app.guiLoop());
    ...
    SECTION("unread count and unread-only filter reflect state") {
        ...
        auto unread = await(notes.execute(bank::dto::ListNotifications{.unreadOnly = true}), app.guiLoop());
