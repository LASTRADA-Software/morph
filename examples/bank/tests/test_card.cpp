...
TEST_CASE("CardModel issues and manages cards", "[card]") {
    ...
    SECTION("issuing a card returns an active card with last-4") {
        auto card = await(cards.execute(bank::dto::IssueCard{.accountId = account,
    ...
    SECTION("freeze/unfreeze and limit/pin changes work") {
        auto card =
            await(cards.execute(bank::dto::IssueCard{.accountId = account, .kind = 0}), app.guiLoop());
        ...
        REQUIRE(await(cards.execute(bank::dto::SetCardLimit{.id = card.id, .dailyLimitMinor = 5000}),
                      app.guiLoop())
                    .ok);
        REQUIRE(
            await(cards.execute(bank::dto::ChangePin{.id = card.id, .newPin = "1357"}), app.guiLoop()).ok);
    }
    ...
    SECTION("a 3-digit PIN is rejected and cancelled cards cannot be unfrozen") {
        auto card =
            await(cards.execute(bank::dto::IssueCard{.accountId = account, .kind = 0}), app.guiLoop());
        REQUIRE_THROWS_AS(
            await(cards.execute(bank::dto::ChangePin{.id = card.id, .newPin = "12"}), app.guiLoop()),
