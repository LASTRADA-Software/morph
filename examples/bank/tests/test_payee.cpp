...
TEST_CASE("PayeeModel add/list/remove scoped to the owner", "[payee]") {
    ...
    SECTION("a valid payee is saved and listed") {
        auto added = await(payees.execute(bank::dto::AddPayee{.name = "Landlord",
    ...
    SECTION("an implausible IBAN is rejected") {
        REQUIRE_THROWS_AS(
            await(payees.execute(bank::dto::AddPayee{.name = "Bad", .iban = "123"}), app.guiLoop()),
            bank::ValidationError);
    }

    SECTION("removing a payee succeeds") {
        auto added = await(payees.execute(bank::dto::AddPayee{.name = "Utility",
...
TEST_CASE("PayeeModel form-style streaming via subscribe/set", "[payee][subscribe]") {
    ...
    REQUIRE(waitUntil([&] { return fired.load(); }, std::chrono::milliseconds{2000},
