...
ReportPresenter::ReportPresenter(::morph::bridge::Bridge& bridge, ::morph::exec::IExecutor* executor,
                                 QObject* parent)
...
void ReportPresenter::submitMonthlyStatement(LedgerId ledgerId, int year, unsigned month,
    ...
    const MonthlyStatementParams params{
        .year = year, .month = month, .timezoneOffsetMinutes = timezoneOffsetMinutes};
    ...
    track<ReportJobId>(
        _handler.execute(SubmitReport{
            .ledgerId = ledgerId, .kind = ReportKind::MonthlyStatement, .params = std::move(paramsJson)}),
        [this](ReportJobId jobId) {
            ...
            _poller = std::make_unique<ReportJobPoller>(
                _bridge, jobId,
                [this](ReportJobId id, ReportJobPoller::OnSuccess onSuccess,
                       ReportJobPoller::OnError onError) {
                    _handler.execute(GetReportStatus{.jobId = id})
                        .then([onSuccess = std::move(onSuccess)](GetReportStatusResult result) {
                            onSuccess(std::move(result));
                        })
                        .onError([onError = std::move(onError)](std::exception_ptr err) {
