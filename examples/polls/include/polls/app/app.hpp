...
///    mechanism at all -- `CreatePoll` mints its own bare
///    admin/participant tokens directly inside `PollModel::execute()`
///    (`polls/auth/polls_authorizer.hpp`'s own `@file` comment). There is
...
///    rung's `PollModel` performs (vote, comment, finalize, undo) is
...
/// worker pool, the `RemoteServer` with a real `auth::PollsAuthorizer`
/// installed, and the durable `FileActionLog` (installed process-wide via
/// `morph::journal::setActionLog`, so every `PollModel` instance
